...
bool AcceptQuestAction::Execute(Event event)
{
    ...
    // These two used to return in silence, which made a bot that never even
    // saw the master's packet look exactly like a bot that refused the quest.
    // Every other way out of AcceptQuest tells the master what happened, so
    // these should too.
    if (!quest || !guid)
    {

...
// CMSG_QUESTGIVER_QUERY_QUEST, so no CMSG_QUESTGIVER_ACCEPT_QUEST is ever
// sent and the bots have nothing to mirror. This one listens to the query
// instead and only acts when the quest really carries QUEST_FLAGS_AUTO_ACCEPT,
// so merely opening the dialog on an ordinary quest still decides nothing.
